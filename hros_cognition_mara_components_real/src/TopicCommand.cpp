...
void HROSCognitionMaraComponentsRealNode::commandCallback(const trajectory_msgs::msg::JointTrajectory::SharedPtr msg)
{
  ...
  for(unsigned int i = 0; i < msg->points[0].positions.size(); i++){
    ...
    if(msg->points[0].velocities.size() > i){
      ...
      RCUTILS_LOG_INFO_NAMED(get_name(), "HROSCognitionMaraComponentsRealNode::commandCallback() you are not defining the velocities!!.");
    }
    cmd_msg1.effort = 0;
    cmd_msg1.control_type = hrim_actuator_rotaryservo_msgs::msg::GoalRotaryServo::CONTROL_TYPE_POSITION;
